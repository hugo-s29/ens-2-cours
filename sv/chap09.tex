...
\begin{document}
  ...
  \begin{defn}
    Pour $M = (S, \mathrm{Act}, {\to}, \mathrm{AP}, L)$, on définit $\llbracket \phi\rrbracket \in \wp(S) $ par induction sur $\phi$ :
     \begin{itemize}
      ...
      \item $\llbracket \lnot \phi \rrbracket := S \setminus \llbracket \phi\rrbracket$ ;
  ...
  \begin{exm}[Logique $\mathsf{LML}$.]
    ...
    De plus, sur ce système, on a $\sigma \sim \sigma'$ ssi $\sigma = \sigma'$.
  \end{exm}
  ...
  \begin{prv}
    ...
    Supposons que $\Phi$ finiement satisfiable dans $\mathrm{Succ}^\alpha(s)$.
