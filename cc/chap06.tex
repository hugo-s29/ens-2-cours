...
\begin{document}
  ...
  \begin{defn}
    ...
    \begin{enumerate}
      \item si $w \in A$ alors $\Pr[\text{$w$ est accepté}] \le 1 - \varepsilon$ ;
      \item si $w \not\in A$ alors $\Pr[\text{$w$ est rejeté}] \le 1 - \varepsilon$.
    \end{enumerate}
  \end{defn}
  ...
  \begin{prop}
    ...
    \begin{enumerate}
      \item si $x \in A$ alors $\Pr_{y \in \{\texttt{0},\texttt{1}\}^{p(|x|)}}[\code{x,y} \in R] \ge \frac{2}{3}$ ;
  ...
  \begin{figure}
    ...
    \addplot +[mark=none,color=deepgreen,thick,dashed] coordinates {(0.3, 0) (0.3, 0.84)};
    \end{axis}
    \end{tikzpicture}
    \caption{Graphe de $x \mapsto 4 x (1-x)$}
  \end{figure}
  % FIXME: Ajoute 1/2 et \varepsilon
  ...
  \begin{prv}
    ...
    Soit $A \in \bpp$.
    Il existe donc $p$ un polynôme et $B \in \p$ tel que, pour tout $x \in \Sigma^n$, on a
    \begin{enumerate}
      \item si $x\in A$ alors 
        \[
          \# \underbrace{\{ y \in \{\texttt{0},\texttt{1}\}^{p(|x|)}  \mid \code{x,y} \in B\}}_{\mathrm{Acc}(x)} \ge \left( 1 - \frac{1}{2^n} \right)\cdot 2^{p(n)} \;
        ;\] 
      \item si $x\not\in A$ alors $\# \mathrm{Acc}(x) \le 2^{p(n)} / 2^n$.
    \end{enumerate}
    Fixons $x \in \{\texttt{0},\texttt{1}\}^n$.
    On doit donc distinguer (avec une formule~$\phSigma 2$) entre deux possibilités
    \begin{enumerate}
      \item l'ensemble $\mathrm{Acc}(x)$ est "très grand" par rapport à l'espace $\{\texttt{0},\texttt{1}\}^{p(n)}$ ;
      \item l'ensemble $\mathrm{Acc}(x)$ est "très petit" par rapport à l'espace $\{\texttt{0},\texttt{1}\}^{p(n)}$.
    \end{enumerate}
