...
\begin{document}
  ...
  \begin{thm}
    Si $s$ est constructible en espace et $s \ge \log n$ alors il existe un langage reconnaissable en espace $\mathrm{O}(s(n))$ mais pas en espace $\mathrm{o}(s(n))$.
  \end{thm}

  \begin{crlr}
    On a les inclusions strictes suivantes :\[
      \underbrace{\dspace{{\log n}}}_{\ls} \subsetneq \dspace{n} \subsetneq \dspace{n^2} \subsetneq \cdots \subsetneq \pspace
    .\] 
  \end{crlr}
  ...
  \begin{note}
    Il manque une partie du cours ici\ldots\ mais, promis, je la recopie bientôt.
  \end{note}
  ...
  \begin{rmk}[Rappels]
    ...
    \begin{itemize}
      ...
      \item et $\nspace{s(n)} \subseteq \dspace{s(n)^2}$ (Savitch)
    \end{itemize}
  ...
  \begin{rmk}
    ...
    On a donc
    \[
      \p \subseteq \np \underset {(*)} \subseteq \pspace \underset {(**)} \subseteq \intro*\exptime = \bigcup_{k \ge 1} \dtime{2^{n^k}}
    ,\] 
    où 
    \begin{itemize}
