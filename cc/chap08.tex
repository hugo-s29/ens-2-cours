...
\begin{document}
  ...
  \begin{defn}
    ...
    x \in A \iff \# \{y \in \Sigma^{\le p(n)}  \mid \code{x,y} \in B\} \text{ est impair}
    .\]
  \end{defn}
  ...
  \begin{defn}
    ...
    \begin{enumerate}
      \item si $x \in A$ alors, $\# \{y \in \{\texttt{0},\texttt{1}\}^{p(n)}  \mid \code{x,y} \in B \} \ge (3/4)\cdot 2^{p(n)}$ ;
      \item si $x \not\in A$ alors, $\# \{y \in \{\texttt{0},\texttt{1}\}^{p(n)}  \mid \code{x,y} \not\in B \} \ge (3/4)\cdot  2^{p(n)}$ ;
    \end{enumerate}
    en notant $n = |x|$.
