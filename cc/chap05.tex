...
\begin{document}
  ...
  \begin{rmk}
    Si $\mathcal{C} \subseteq \mathcal{C}'$ pour deux classes de langages $\mathcal{C}$, $\mathcal{C}'$, alors on a l'inclusion $\co{\mathcal{C}} \subseteq \co{\mathcal{C}'}$.
    De plus, on a que $\co{(\co{\mathcal{C}})} = \mathcal{C}$.
  \end{rmk}
  ...
  \[
  \begin{tikzcd}[column sep={38,between origins}, row sep={35, between origins}]
    ...
    \mathclap{\p} \arrow[marking,phantom]{ur}{\subseteq} \arrow[marking,phantom]{dr}{\subseteq} && \mathclap{\phDelta 2} \arrow[marking,phantom]{ur}{\subseteq} \arrow[marking,phantom]{dr}{\subseteq} && \mathclap{\phDelta 3} \arrow[marking,phantom]{ur}{\subseteq} \arrow[marking,phantom]{dr}{\subseteq} && \mathclap{\cdots} \arrow[marking,phantom]{r}{\subseteq} &\mathclap{\ph} \arrow[marking,phantom]{r}{\subseteq}& \quad\;\mathclap{\pspace}\\
    & \phPi 1 \arrow[marking,phantom]{ur}{\subseteq} = \conp && \phPi 2 \arrow[marking,phantom]{ur}{\subseteq} && \phPi 3 \arrow[marking,phantom]{ur}{\subseteq}\\
  \end{tikzcd}
  \]
  ...
  \begin{rmk}
    ...
    Les classes $\phDelta i$ sont closes par complément. Par exemple, l'inclusion $\phDelta i \subseteq \phPi i$ découle de la clôture par complément et de l'inclusion $\phDelta i \subseteq \phSigma i$.

    On omet parfois l'exposant "$\mathrm{P}$", mais attention, il existe une hiérarchie similaire (avec $\Sigma_i$, $\Pi_i$ et $\Delta_i$) en calculabilité.
  \end{rmk}
  ...
  \begin{thm}
    ...
    \[
      ...
      \end{pmatrix}
    ,\]
    avec une alternance de $\forall$ et de $\exists$, commençant par un $\exists$.\footnote{On note ici $\mathsf{Q}_i := \exists$ si $i$ est impair et $\mathsf{Q}_i := \forall$ si $i$ est pair.}
    \qed
  \end{thm}
  ...
  \begin{prv}
    ...
    \[
      C := \mleft\{\, \code{x, y_1} \;\middle|\; \forall y_2 \ldots \mathsf{Q}_{i+1} y_{i+1} \code{x, y_1, \ldots, y_{i+1}}  \in B \,\mright\} 
    \]
    est dans $\phPi i$, donc dans $\phSigma i$.
    D'où, par caractérisation,
     \[
       x \in A \iff \exists y_1 \: \underbrace{\exists z_1 \: \forall z_2 \: \ldots \: \mathsf{Q}_i z_i \: \code{x, y_1, z_1, \ldots, z_i} \in D}_{\code{x, y_1} \in C}
    ,\]
    avec $D \in \p$. Et ainsi $A$ est un problème de $\phSigma i$ en combinant les deux "$\exists$" (avec la remarque précédente).
  \end{prv}
  ...
  \begin{defn}
    Un circuit booléen à $s$ entrées \textit{décide \pbSat}\ si, étant donnée une formule booléenne $F$ de taille  $s$, le circuit $C$ décide si $F$ est satisfiable.
  \end{defn}
  ...
  \end{lem}
  \begin{prv}
    On utilise le fait que \pbSat\ est \textit{\textbf{auto-réductible}}\footnote{\textit{self-reductible} en anglais.} : une formule booléenne $F(v_1, \ldots, v_n)$ est satisfiable si et seulement si l'une des deux formules booléennes
    \[
    ...
    \begin{enumerate}
      ...
      \item si $F$ dépend de $n \ge 1$ variables $v_1, \ldots, v_n$ alors $C(F) = \texttt{1}$ ssi
        \[
          C(F[v_n := \texttt{0}]) = \texttt{1} \quad \text{ ou }\quad C(F[v_n := \texttt{1}]) = \texttt{1}
        .\]
    \end{enumerate}
  ...
  \begin{prv}[du théorème de Karp-Lipton]
    ...
    est équivalente à 
    \[
    \exists C \: \forall u \quad C(F(u, \cdot)) = \texttt{1} \quad \text{ et }\quad \text{$C$ décide \pbSat}
    ,\]
    ...
    \[
      \exists C \: \forall u \quad C(F(u, \cdot)) = \texttt{1} \quad \text{ et } \quad \forall y \in \{\texttt{0},\texttt{1}\}^{p(s)} \: \code{C, y} \in A
    ,\] 
    avec $A \in \p$ d'après le lemme~\ref{lem:karp-lipton-lem1} et la caractérisation de \conp.
    On en déduit que ceci est équivalent à 
    \[
      \exists C \: \forall u \: \forall y \quad \underbrace{C(F(u, \cdot )) = \texttt{1} \quad \text{ et } \quad \code{C, y} \in A}_{\text{vérifiable en temps polynomial}}
    ,\]
    et est donc vérifiable dans $\phSigma 2$ grâce à la caractérisation par quantificateurs.
  \end{prv}
