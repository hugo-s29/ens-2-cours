...
\begin{document}
  ...
  We can create an accepting execution $t$ for $w := u 0^\ell \bar{v}$ where $\ell = (i-k)+(2k -j) = k+i-j$: start like $r$, and whenever you cross from cell $i$ to cell $i+1$, act like $s$, and whenever you cross from $j+1$ to $j$, act like $s$ again.
  This execution is, indeed, accepting, as either $r$ will accept while its head is before $i$ (and so the execution $t$ behaves like $r$), either $t$ will accept while its head is after $j$ (and so $t$ behaves like $s$).
  ...
  As each element of some $C_i(y)$ is a step (and we don't have duplicates when considering different $i$s), we know that
   \[
  ...
  By question 2, we can associate some $i = i(y)$ such that $|C_{i(y)}| \le T(3k) / k$ to every $y \in \mathsf{PaddedPal}_k$.
  ...
  k^2 \underbrace{\left( \frac{\log |\Sigma|}{\log|Q|} - \frac{1}{k} \right)}_{9\alpha} \le T(3k)
  .\]
  ...
  Then, consider the following language on $\{\texttt{0}, \texttt{1}, \texttt{\#}\}$:
  \[
  ...
  start by writing $w$ on $N$'s input tape, compute $C \cdot 2^{n^k}$ (which is time-constructible by Tutorial 3, Exercise 2) in unary (using the \texttt{\#} character), adding that on the input tape after $w$, and finally start the execution of~$N$.
  ...
  As
  \[
    \nspace{n^s} = \nspace{n^t} \subseteq \nspace{n^{t+1}} \subseteq \cdots \subseteq \nspace{n^s}
  \]
  ...
  \[
    A := \mleft\{\,w \texttt{\#}^{n^{(t + 2)/(t + 1)} - n} \;\middle|\; w \in L(M), n = |w|\,\mright\} 
  \] is in $\nspace{m^{t+1}}$ where $M$ is a non-deterministic Turing machine that uses at most $\mathrm{O}(n^{t+1})$ space.
  Thus, we can show that $L(M) \in \nspace{n^{t+1}}$ as we can simulate $M$ on $\bar{w}$ using
  \[
    \mathrm{O}(m^{t+1}) = \mathrm{O}\big((n^{(t+1) / (t+2)})^{t+1}\big) = \mathrm{O}(n^{t+1})
  \] 
  space.
  Doing this for $s = t+1, t+2, t+3, \ldots$, we obtain that 
  \begin{equation}
    \nspace{n^t} = \nspace{n^{t+1}} = \cdots = \nspace{n^{t + k}} = \cdots,
    \tag{$*$}
    \label{eq:pb2-contra}
\end{equation}
  and thus $\dspace{n^{2t}} = \dspace{n^{2(t+1)}}$ which contradicts the space hierarchy ($n^{2(t+1)}$ is space-contractible and $n^{2t} = \mathrm{o}(n^{2(t+1)})$).
  ...
  \begin{note}
    ...
    ,\] 
    and thus we get the contradiction $\pspace = \dspace{n^t}$.
  \end{note}
\end{document}
