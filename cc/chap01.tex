...
\begin{document}
  ...
  \begin{defn}
    Une \textit{machine universelle} $U$ prend en entrée des couples $\code{x,\alpha}$ (où $\alpha$ est le code d'une machine $M_\alpha$ et $x \in \Sigma^\star$) et simule la machine $M_\alpha$ sur l'entrée $x$.
  ...
  \begin{thm}
    Il existe une machine de Turing universelle $U$ telle que, sur toute entrée $\code{x, \alpha}$, si  $M_\alpha$ s'arrête sur $x$ en $t$ étapes, alors la machine $U$ s'arrête sur $\code{x, \alpha}$ en au plus $c\: t \log t$, où $c$ ne dépend que de $x$.
  \end{thm}
  \begin{prv}
    \begin{itemize}
