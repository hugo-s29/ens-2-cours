...
def issueRouteHead(index1, index2):
	return f"""\t<route src="host-{index1}.{hostname}" dst="host-{index2}.{hostname}" symmetrical="NO">\n"""
...
if (len(sys.argv) != 6):
	print("Usage: smpi-generate-ring.py <num hosts> <real-machine-compute-power> <simulation-node-compute-power> <simulation-link-bandwidth> <simulation-link-latency> \n", file=sys.stderr)
	print("Example: smpi-generate-ring.py 32 1000 100 10Gbps 10us \n", file=sys.stderr)
	print("  Will generate a ring-<num hosts>-platform.xml and ring-<num hosts>-hostfile.txt file\n", file=sys.stderr)
	exit(1)
...
filename = f"./ring-{num_hosts}-platform.xml"
with open(filename, 'w') as fh:
	...
	print("Ring XML platform file created: "+filename, file=sys.stderr)

###############################################################
## Generate host file
filename = f"./ring-{num_hosts}-hostfile.txt"
